...
class MonthPickerDialog(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ...
        for month in range(1, 13):
            row = ((month - 1) % 6) + 1
            column = 0 if month <= 6 else 1
            ctk.CTkButton(
                self,
                text=f"{MONTH_LABELS[month].capitalize()} {self._year}",
                height=30,
                corner_radius=8,
            ...
            ).grid(row=row, column=column, padx=10, pady=4, sticky="ew")
...
class TransactionListPage(BasePage):
    def __init__(self, master, controller, tx_type: int, title: str) -> None:
    ...
    def refresh(self) -> None:
        ...
        first_name = summary["username"].split()[0]
        self.greeting_label.configure(text=f"Hello {first_name}")
        self.date_label.configure(text="May 19, 2023")
    ...
    def _build_header(self) -> None:
        ...
        separator = ctk.CTkFrame(header, height=1, fg_color="#59616E")
        separator.grid(row=2, column=0, sticky="ew", pady=(8, 0))
    ...
    def _build_filters(self) -> None:
        ...
        self.month_button = self._create_chip(chips_row, "", active=False, command=self._open_month_dialog, width=110)
    ...
    def _build_table(self) -> None:
        ...
        self._header_label(self.table_header, 0, "Дата", "w")
        self._header_label(self.table_header, 1, "Описание", "w")
        self._header_label(self.table_header, 2, "Категории", "w")
        amount_title = "Сумма" if self.tx_type == INCOME else "Количество"
        self._header_label(self.table_header, 3, amount_title, "e")
    ...
    def _render_filters(self) -> None:
        ...
        all_button = self._create_chip(
            self.filters_wrap,
            "все категории",
            active=self.selected_category_id is None,
            command=lambda: self._select_category(None),
        )
        all_button.pack(side="left", padx=(0, 10))
        self.filter_buttons[None] = all_button

        for category in self.repo.get_categories(self.user["id"], self.tx_type):
            button = self._create_chip(
                self.filters_wrap,
                category["name"],
                active=self.selected_category_id == category["id"],
                command=lambda category_id=category["id"]: self._select_category(category_id),
            )
            button.pack(side="left", padx=(0, 10))
            self.filter_buttons[category["id"]] = button
    ...
    def _create_chip(self, parent, text: str, active: bool, command=None, width: int = 96) -> ctk.CTkButton:
        return ctk.CTkButton(
            ...
            border_width=1,
            border_color="#3FC2A3" if active else "#233044",
    ...
    def _render_rows(self) -> None:
        ...
        for index, row in enumerate(rows):
            ...
            row_frame.grid(row=index, column=0, sticky="ew", pady=(0, 2))
            row_frame.grid_propagate(False)
            row_frame.grid_columnconfigure(0, weight=0, minsize=138)
            row_frame.grid_columnconfigure(1, weight=1, minsize=280)
            row_frame.grid_columnconfigure(2, weight=0, minsize=190)
            row_frame.grid_columnconfigure(3, weight=0, minsize=134)

            self._row_text(row_frame, 0, self._format_date(row["created_at"]), "#F4F6FA", "w", 12)
            self._row_text(row_frame, 1, row["description"] or "-", "#9AA5B5", "w", 16)
            self._row_badge(row_frame, 2, row["category_name"])
            self._row_text(row_frame, 3, self._format_amount(row["amount"]), self.amount_color, "e", 13, "bold")
    ...
    def _get_month_label(self) -> str:
        if self.selected_year is None or self.selected_month is None:
            return ""
        return f"{MONTH_LABELS[self.selected_month]} {self.selected_year}"
    ...
    def _ensure_selected_month(self) -> None:
        ...
        if latest is None:
            ...
            dt = datetime.strptime(latest["created_at"], "%Y-%m-%d %H:%M:%S")
    ...
    def _format_date(self, value: str) -> str:
        ...
        return f"{dt.day:02d} {MONTH_SHORT[dt.month]},{dt.year}"
